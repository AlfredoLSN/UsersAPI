...
class User{
    ...
    async findAll(){
        try{
            var result = await knex.select(["id", "email", "role", "name"]).from("users");
            return result;
        }catch(err){
            console.log(err);
            return [];
    ...
    async findById(id){
        try{
            var result = await knex.select(["id", "email", "role", "name"]).where({id: id}).from("users");
            if(result.length > 0){
                return result[0];
            }else{
                return undefined;
    ...
    async findByEmail(email){
        try{
            var result = await knex.select(["id", "email", "password", "role", "name"]).where({email: email}).from("users");
            if(result.length > 0){
                return result[0];
            }else{
                return undefined;
